...
There are multiple implementations of `aggregate` provided. For most users, the `aggregate_numpy.py` implementation is probably the easiest to install and offers fairly reasonable speed for the majority of aggregation functions.  If you simply use `from aggregate import aggregate`, the best available implementation will automatically be selected.

Currently the following implementations exist:
* **numpy** - This implementation uses plain `numpy`, mainly relying on `np.bincount` and basic indexing magic. It comes without other dependencies except `numpy` and shows reasonable performance.
* **numba** - *Use, if you have numba available.* This is the most performant implementation for a wide range of functions. As `numba` can raise errors from inside jitted functions, boundary checks can be inlined and the `weave` implementation gets beaten in several benchmarks.
* **weave** - *Use, if you have a working GCC setup and no numba.* `weave` compiles C-code on demand at runtime, producing (and caching) binaries that get executed from within python. This is currently the fastest of our implementations, especially for nan-functions.
